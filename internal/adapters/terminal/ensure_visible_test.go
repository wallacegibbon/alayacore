...
func TestEnsureCursorVisible_OversizedWindowDoesNotOscillate(t *testing.T) {
	...
	wb.AppendOrUpdate(stream.TagAssistantT, "window-1", "Small window")
	wb.AppendOrUpdate(stream.TagAssistantT, "window-2", strings.Repeat("line\n", 100))
...
func TestEnsureCursorVisible_OversizedWindowScrollsWhenOffScreen(t *testing.T) {
	...
	wb.AppendOrUpdate(stream.TagAssistantT, "window-2", "Tail")
...
func TestEnsureCursorVisible_PartiallyVisibleWindowNotMoved(t *testing.T) {
	...
	for i := range 20 {
		wb.AppendOrUpdate(stream.TagAssistantT, "window-"+strings.Repeat("x", i+1),
			strings.Repeat("line\n", 5))
	}
...
func TestAutoFollow_OnlyGEnables(t *testing.T) {
	...
	wb.AppendOrUpdate(stream.TagAssistantT, "w1", strings.Repeat("line\n", 5))
	wb.AppendOrUpdate(stream.TagAssistantT, "w2", strings.Repeat("line\n", 5))
...
func TestAutoFollow_JNoOpWhenNewWindowBetweenTicks(t *testing.T) {
	...
	wb.AppendOrUpdate(stream.TagAssistantT, "w4", strings.Repeat("line\n", 5))
...
func TestScrollCursorToTop_PositionsWindowAtTop(t *testing.T) {
	...
	for i := range 10 {
		wb.AppendOrUpdate(stream.TagUserT, "prompt-"+strings.Repeat("x", i+1),
			strings.Repeat("line\n", 5))
		wb.AppendOrUpdate(stream.TagAssistantT, "response-"+strings.Repeat("x", i+1),
			strings.Repeat("line\n", 5))
	}
...
func TestScrollCursorToTop_PartiallyVisibleWindowMovesToTop(t *testing.T) {
	...
	// Create windows
	for i := range 10 {
		wb.AppendOrUpdate(stream.TagUserT, "window-"+strings.Repeat("x", i+1),
			strings.Repeat("line\n", 5))
	}
...
func TestScrollCursorToTop_NoCursorDoesNothing(t *testing.T) {
	...
	wb.AppendOrUpdate(stream.TagUserT, "window-1", "hello")
	display.SetHeight(10)
	display.updateContent()
