...
func TestWindowBufferDiff(t *testing.T) {
	t.Run("append diff content", func(t *testing.T) {
		...
		wb.HandleFunctionEvent(stream.FunctionData{ID: "diff-1", Type: "call", Name: "edit_file", Input: content})
	...
	t.Run("diff window folds when wrapped", func(t *testing.T) {
		...
		wb.HandleFunctionEvent(stream.FunctionData{ID: "diff-1", Type: "call", Name: "edit_file", Input: content.String()})
...
func TestWindowBufferVisibility(t *testing.T) {
	t.Run("tool windows are always visible", func(t *testing.T) {
		wb := NewWindowBuffer(80, DefaultStyles())
		wb.HandleFunctionEvent(stream.FunctionData{ID: "tool-1", Type: "call", Name: "execute_command", Input: ""})
